...
require('./config')
const { default: hisokaConnect, useSingleFileAuthState, DisconnectReason, fetchLatestBaileysVersion, generateForwardMessageContent, prepareWAMessageMedia, generateWAMessageFromContent, generateMessageID, downloadContentFromMessage, makeInMemoryStore, jidDecode, proto, mentionedJid } = require("@adiwajshing/baileys")
...
// (id == null || id == undefined || id == false) ?  hisoka.sendMessage(m.chat, { text: teks, contextInfo: { "mentionedJid": memberr }}):  xdev.sendMessage(m.chat, { text: teks, contextInfo: { "mentionedJid": memberr }},{quoted: m})
// }
...
async function startHisoka() {
    ...
    console.log(chalk.bold.green(figlet.textSync('BOT WA', {
		font: 'Standard',
	...
	console.log(color('[S T A T U S]', 'cyan'), color('BOT ONLINE', 'yellow'))
console.log(color('[W A R N I N G]', 'cyan'), color(`INGFO CREATOR`, 'yellow'))
console.log(color('Instagram :', 'cyan'), color(`@zyee_ez`, 'yellow'))
console.log(color('Github :', 'cyan'), color(`@takimtod`, 'yellow'))
console.log(color('Wangsap :', 'cyan'), color(`082194424412`, 'yellow'))
	console.log(color('\n', 'cyan'), color('\nSCRIPT INI DI PANTAU PKI, JADI AWAS AJA  !!!', 'yellow'))
    ...
    hisoka.sendMessage(`${global.pemilik}`, {text: `*Lapor Bot:* Ada Yang Menelpon Bot`})
    await sleep(8000)
    ...
    hisoka.ev.on('messages.upsert', async chatUpdate => {
        ...
        require("./kimmy")(hisoka, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
...
ngen =  await getBuffer(ppuser)
tod =  await getBuffer(ppuser)

if (anu.action == 'add') {
hisoka.send5Loc(anu.id, cangton, `Moga Betah Yak `, ngen, btn, {contextInfo:{ "mentionedJid": [num]}})
} else if (anu.action == 'remove') {
hisoka.send5Loc(anu.id, cangtion, `Bye-Bye Kawand `, tod, bton, {contextInfo:{ "mentionedJid": [num]}})
}
   }
       } catch (err) {
            console.log(err)
     ...
     * @returns
     */
    hisoka.send5Vid = async (jid , text = '' , footer = '', vid, but = [], options = {}) =>{
    ...
    hisoka.send5Loc = async (jid , text = '' , footer = '', img, but = [], options = {}) =>{
        var template = generateWAMessageFromContent(m.chat, proto.Message.fromObject({
        templateMessage: {
        hydratedTemplate: {
               "hydratedContentText": text,
